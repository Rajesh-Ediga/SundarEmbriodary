...
let selected='All';
const filters=document.querySelector('#categoryFilters'),grid=document.querySelector('#productGrid'),designTitle=document.querySelector('#design-title'),designStatus=document.querySelector('#designStatus');
...
grid.addEventListener('click',event=>{const card=event.target.closest('[data-id]');if(card)openProduct(card.dataset.id)});
